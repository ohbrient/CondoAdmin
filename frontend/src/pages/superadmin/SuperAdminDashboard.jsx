...
export default function SuperAdminDashboard() {
  ...
  const load = () => Promise.all([
    ...
    // Parse admins si viene como string JSON desde el backend
                        ...
                        {c.admins?.length > 0
                          ? c.admins.map(a => (
                              <div key={a.id} style={{ display:'flex', alignItems:'center', gap:'6px', marginBottom:'2px' }}>
                                <div style={{ width:'20px', height:'20px', borderRadius:'50%', background:'var(--accent)', display:'flex', alignItems:'center', justifyContent:'center', fontSize:'9px', fontWeight:'700', color:'#fff', flexShrink:0 }}>
          ...
          <div className="modal" style={{ maxWidth:'440px' }}>
            ...
            {modalAdmins.admins?.length > 0 && (
              <div style={{ marginBottom:'20px' }}>
                <div style={{ fontSize:'11px', color:'var(--text2)', textTransform:'uppercase', letterSpacing:'0.5px', marginBottom:'8px', fontWeight:'600' }}>Admins actuales</div>
            ...
            <div style={{ marginBottom:'20px' }}>
              ...
              <select className="input" value={adminSeleccionado} onChange={e => setAdminSeleccionado(e.target.value)}>
                ...
                {usuarios.map(u => (
                  <option key={u.id} value={u.id}>{u.nombre} {u.apellido} · {u.email} ({u.rol})</option>
                ))}
              </select>
              <p style={{ fontSize:'12px', color:'var(--text2)', marginTop:'6px' }}>
                Si el usuario no aparece, créalo primero en <Link to="/superadmin/usuarios" style={{ color:'var(--accent)' }}>Usuarios</Link>.
              </p>
            </div>
