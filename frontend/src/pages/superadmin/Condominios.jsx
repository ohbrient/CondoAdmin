...
export default function SACondominios() {
  ...
  const load = () => {
    api.get('/condominios').then(r => setCondos(r.data)).catch(console.error).finally(() => setLoading(false));
  };
              ...
              <div key={c.id} className="card" style={{ position:'relative', overflow:'hidden' }}>
                ...
                <div style={{ padding:'24px' }}>
                  ...
                  <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'8px', marginBottom:'20px' }}>
                    {[
                      ['Unidades creadas', `${c.total_unidades || 0}${c.max_unidades ? ' / ' + c.max_unidades : ''}`],
                  ...
                  <div style={{ display:'flex', gap:'8px' }}>
                    ...
                    <button className="btn btn-ghost" style={{ flex:1, justifyContent:'center' }} onClick={() => toast('Función de admins próximamente')}>👥 Admins</button>
                  </div>
                </div>
              </div>
            ))}
